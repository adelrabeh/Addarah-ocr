...
| `artifacts/darah-ocr-dotnet` | 6000 | — | **ASP.NET Core 8 + Blazor Server** OCR system |

### .NET System (artifacts/darah-ocr-dotnet)

- **Stack**: ASP.NET Core 8, Blazor Server (Interactive), Entity Framework Core 8, Npgsql PostgreSQL
- **Auth**: Cookie-based session, BCrypt password hashing, brute-force lockout (5 attempts / 15 min)
- **Database tables**: `dotnet_users`, `dotnet_jobs`, `dotnet_ocr_results`, `dotnet_audit_logs`, `dotnet_api_keys`
- **Default credentials**: `admin / Admin@1234`, `operator / Operator@1234`
- **OCR Engine**: Gemini 2.0 Flash via HTTP (reads GEMINI_API_KEY env var)
- **Features**: Upload → OCR → Review → Approve, Blazor Server real-time notifications, full-text search, DOCX export, bulk ZIP, API keys, audit logs
- **Workflow**: `artifacts/darah-ocr-dotnet: .NET OCR System` running on port 6000
