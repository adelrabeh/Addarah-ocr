...
export default function JobDetailPage() {
  ...
  const progressInfo = jobProgress[id];

  // Parse uncertain words from rawText — Gemini marks them with [؟] or [كلمة]
  const uncertainWords = useMemo(() => {
    const raw = (result as (typeof result & { rawText?: string }) | null)?.rawText ?? result?.refinedText ?? "";
    if (!raw) return { markedWords: [] as string[], unreadableCount: 0 };
    const markedWords: string[] = [];
    const wordPattern = /(\S+)\s*\[؟\]/g;
    let match;
    while ((match = wordPattern.exec(raw)) !== null) {
      markedWords.push(match[1] ?? "");
    }
    const unreadableCount = (raw.match(/\[كلمة\]/g) ?? []).length;
    return { markedWords: [...new Set(markedWords)].slice(0, 60), unreadableCount };
  }, [result]);
              ...
              <div className="w-full mt-2 space-y-2">
                <div className="flex items-center gap-2 text-sm text-blue-600">
                  ...
                  {progressInfo && progressInfo.totalPages > 0 ? (
                    <span>معالجة الصفحة {progressInfo.processingPage} من {progressInfo.totalPages}</span>
                  ) : (
                    <span className="animate-pulse">جاري تجهيز الملف...</span>
                  )}
                </div>
                {progressInfo && progressInfo.totalPages > 0 && (
                  <div className="space-y-1">
                    <Progress
                      value={Math.round((progressInfo.processingPage / progressInfo.totalPages) * 100)}
                      className="h-2"
                    />
                    <p className="text-xs text-muted-foreground text-left ltr">
                      {Math.round((progressInfo.processingPage / progressInfo.totalPages) * 100)}%
                      — تقريباً {Math.ceil((progressInfo.totalPages - progressInfo.processingPage) / 5 * 30)} ثانية متبقية
                    </p>
                  </div>
                )}
              </div>
            ...
            <>
              ...
              <Card className="shadow-sm">
                ...
                <CardContent className="space-y-4">
                  ...
                  {/* Uncertain words parsed from OCR output [؟] markers */}
                  {(uncertainWords.markedWords.length > 0 || uncertainWords.unreadableCount > 0) && (
                    <div className="border-t pt-4">
                      <div className="flex items-center gap-2 mb-3">
                        <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0" />
                        <p className="text-sm font-semibold text-amber-700">
                          كلمات تحتاج مراجعة
                          {uncertainWords.markedWords.length > 0 && (
                            <span className="mr-1.5 text-xs font-normal text-muted-foreground">
                              ({uncertainWords.markedWords.length} كلمة مشكوك فيها
                              {uncertainWords.unreadableCount > 0 && `، ${uncertainWords.unreadableCount} غير مقروءة`})
                            </span>
                          )}
                        </p>
                      </div>
                      {uncertainWords.markedWords.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {uncertainWords.markedWords.map((word, i) => (
                            <span
                              key={i}
                              className="inline-flex items-center gap-1 px-2.5 py-1 bg-orange-50 border border-orange-200 rounded-full text-xs font-medium text-orange-800"
                            >
                              {word}
                      ...
                      {uncertainWords.unreadableCount > 0 && (
                        <p className="text-xs text-red-600 bg-red-50 border border-red-100 rounded px-3 py-2">
                          يوجد {uncertainWords.unreadableCount} كلمة {uncertainWords.unreadableCount === 1 ? "غير مقروءة تماماً" : "غير مقروءة تماماً"} — تحتاج مراجعة يدوية
                        </p>
                      )}
                    </div>
                  )}
