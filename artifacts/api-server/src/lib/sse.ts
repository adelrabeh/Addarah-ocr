...
export interface SseEvent {
  type: "job_ready_for_review" | "job_ready_for_approval" | "job_approved" | "job_rejected" | "job_completed" | "job_progress";
  jobId: number;
  filename: string;
  message: string;
...
export function broadcastJobProgress(jobId: number, filename: string, processingPage: number, totalPages: number): void {
  broadcastToRole(
    {
      type: "job_progress",
      jobId,
      filename,
      message: `معالجة الصفحة ${processingPage} من ${totalPages}`,
      processingPage,
      totalPages,
    },
    () => true,
