...
import { notifyJobOcrComplete, broadcastJobProgress } from "./sse";
...
async function processJobById(jobId: number): Promise<void> {
  ...
  try {
    ...
    // Progress callback — updates DB + broadcasts SSE to all connected clients
    const onProgress = async (completedPage: number, totalPages: number) => {
      await db
        .update(jobsTable)
        .set({ processingPage: completedPage, totalPages })
        .where(eq(jobsTable.id, jobId));
      broadcastJobProgress(jobId, job.filename, completedPage, totalPages);
    };

    // Run OCR engine
    const result = await processOcr(job.filename, onProgress);
